...
int init() 
{
    ...
    // 设置数据采样频率
    printf("Setting MPU sample rate...\n");
    if (mpu_set_sample_rate(100) != 0) {
        printf("Failed to initialize MPU  sample rate!\n");
        return -1;
    }

    // 设置低通滤波器频率
    printf("Setting MPU lowpass filter...\n");
    if (mpu_set_lpf(20) != 0) {
        printf("Failed to initialize MPU lowpass filter!\n");
        return -1;
    }
...
int UpdateRawData()
{
    int16_t g_gyro[3];
    int16_t g_accel[3];
	uint8_t g_sensors, g_more;
    int read_counter = 0;
    while( mpu_read_fifo(g_gyro, g_accel, &g_sensors, &g_more) == 0){
        read_counter++;
        printf("g_gyro: %05d %05d %05d ", g_gyro[0], g_gyro[1], g_gyro[2]);
        printf("g_accel: %05d %05d %05d\n", g_accel[0], g_accel[1], g_accel[2]);
    }
    return read_counter;
}
